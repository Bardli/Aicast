...
class User(Base):
    __tablename__ = "users"
    user_id = Column(
        UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, server_default=sqlalchemy.text("gen_random_uuid()")
    )
...
class PodcastBlock(Base):
    ...
    podcast_id = Column(
        UUID(as_uuid=True), ForeignKey("podcasts.podcast_id", ondelete="CASCADE"), nullable=False
    )
    block_id = Column(
        UUID(as_uuid=True), ForeignKey("blocks.block_id", ondelete="CASCADE"), nullable=False
    )
...
class Article(Base):
    ...
    podcast_id = Column(
        UUID(as_uuid=True), ForeignKey("podcasts.podcast_id", ondelete="CASCADE"), nullable=False, index=True
    )
    source_block_id = Column(
        UUID(as_uuid=True), ForeignKey("blocks.block_id", ondelete="SET NULL"), nullable=False
    )
